...
class PushDispatcher(TaskDispatcher):   
    ...
    def start(self):
        """Deploy push dispatcher load balacing by workers
        """
        free_workers = deque()
        while True:
            ...
            # a 'register' message. Afterwards, available threads are identified when 
            ...
            if self.socket in sockets:
                ...
                # if result, update database and append worker process to list of workers ready to receive tasks
                    ...
                    # update worker's number of free processes
                    self.workers[worker_id].free_processes += 1

                    # if the worker was not in the list of available workers, add it
            ...
            if free_workers:
                ...
                if redis_message is not None and redis_message['type'] == 'message':
                    ...
                    #  send task to worker and update status
                    ...
                    # if worker still has free processes, add it to the list of available workers 
                    if self.workers[worker_id].free_processes > 0:
                        free_workers.append(worker_id)
    
    def start_heartbeat(self):
        """Deploy push dispatcher load balacing by workers, with heartbeat
        """
    ...
    def start_proc_load_balance(self):
        """Deploy push dispatcher load balacing in the processes
        """
        free_workers = deque()
        #last_shuffle = 0
        while True:
            ...
            if self.socket in sockets:
                ...
                # if 'register', add worker processes to list of available workers
                if worker_message['type'] == 'register':
                    num_processes = worker_message['data']['num_processes']
            ...
            # shuffle list of available workers
            # if time.time() - last_shuffle > 0.1:
...
if __name__ == "__main__":
    ...
    if args.m ==  'local':
        ...
        if args.m == 'pull':
            ...
            time.sleep(args.d)
            if args.hb:
                #print("Starting push dispatcher, heartbeat mode")
                dispatcher.start_heartbeat()
            else:
                if args.plb:
                    #print("Starting push dispatcher, process load balancing mode")
                    dispatcher.start_proc_load_balance()
                else:
